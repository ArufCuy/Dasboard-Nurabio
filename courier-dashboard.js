...
function updateCourierInfo() {
  document.getElementById("courier-name").textContent = `Halo, ${currentCourier.name}!`
  document.getElementById("total-deliveries").textContent = currentCourier.totalDeliveries || 0
}
...
function setupOrderListeners() {
  ...
  const availableOrdersQuery = window.query(window.collection(window.db, "orders"), window.orderBy("createdAt", "desc"))
  ...
  window.onSnapshot(availableOrdersQuery, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
      ...
      } else if (!orderData.assignedCourierId && orderData.status === "Pending") {
...
function createAvailableOrderCard(order) {
  ...
  card.className = "bg-white/5 border border-white/10 rounded-lg p-4 animate-fade-in"
  ...
  card.innerHTML = `
    ...
    <div class="border-t border-white/10 pt-3 space-y-2">
      ...
      <div class="flex items-start gap-2">
        ...
        <span class="text-gray-300 text-sm">${order.customerAddress || "No address"}</span>
      </div>
...
window.takeOrder = async (orderId) => {
  try {
    ...
    const customerDoc = await window.getDocs(
      window.query(window.collection(window.db, "customers"), window.where("name", "==", order.customerName)),
    )
    ...
    await window.updateDoc(window.doc(window.db, "couriers", currentCourier.id), {
...
window.updateOrderStatus = async (orderId, newStatus) => {
  try {
    ...
    if (newStatus === "Selesai") {
      ...
      currentCourier.totalDeliveries = (currentCourier.totalDeliveries || 0) + 1
      localStorage.setItem("courierData", JSON.stringify(currentCourier))
      updateCourierInfo()
    }

    await window.updateDoc(window.doc(window.db, "orders", orderId), updateData)
...
function updateStats() {
  ...
  const completedToday = myOrders.filter((order) => {
    return order.completedAt && new Date(order.completedAt).toDateString() === today
  }).length
...
function showToast(message, type = "success") {
  ...
  if (type === "error") {
    ...
    toastContainer.className = "bg-blue-500 text-white px-4 py-2 rounded-lg shadow-lg"
  } else {
