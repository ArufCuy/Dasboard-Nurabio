...
function updateCourierInfo() {
  ...
  // Calculate total gallons delivered
  const totalGallons = myOrders
    .filter((order) => order.status === "Selesai")
    .reduce((sum, order) => sum + (order.quantity || 0), 0);

  document.getElementById("total-deliveries").innerHTML = `
    <div class="text-2xl font-bold text-white">${
      currentCourier.totalDeliveries || 0
    }</div>
    <div class="text-sm text-gray-400">${totalGallons} galon total</div>
  `;
}
...
function updateStats() {
  ...
  // Update total deliveries to show gallons too
  const totalGallons = myOrders
    .filter((order) => order.status === "Selesai")
    .reduce((sum, order) => sum + (order.quantity || 0), 0);
