...
function createCourierCard(courier) {
  ...
  // Calculate total gallons delivered by this courier
  ...
  const totalGallons = courierOrders.reduce(
    (sum, order) => sum + (order.quantity || 0),
    0
  );

  // Calculate gallons this week
  const weekStart = new Date();
  weekStart.setDate(weekStart.getDate() - weekStart.getDay());
  const weeklyGallons = courierOrders
    .filter((order) => {
  ...
  // Calculate gallons today
  const today = new Date().toDateString();
  const dailyGallons = courierOrders
    .filter((order) => {
      const orderDate = new Date(order.completedAt || order.updatedAt);
    ...
    courier.isActive
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-2 pt-2 border-t border-white/10 text-sm">
                ...
                <div>
                    <span class="text-gray-400">Total Galon:</span>
                    <div class="text-blue-400 font-medium">${totalGallons} galon</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                    <span class="text-gray-400">Galon Minggu:</span>
                    <div class="text-green-400 font-medium">${weeklyGallons} galon</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                    <span class="text-gray-400">Galon Hari:</span>
                    <div class="text-purple-400 font-medium">${dailyGallons} galon</div>
                </div>
            </div>
...
function updateCourierStats() {
  ...
  // Calculate total gallons delivered by all couriers
  const completedOrders = orders.filter((order) => order.status === "Selesai");
  const totalGallons = completedOrders.reduce(
    (sum, order) => sum + (order.quantity || 0),
    0
  );
  ...
  // Update the courier stats section to show gallons if element exists
  const weeklyGallonsEl = document.getElementById("couriers-weekly-gallons");
  if (weeklyGallonsEl) {
    const weekStart = new Date();
    weekStart.setDate(weekStart.getDate() - weekStart.getDay());
    const weeklyGallons = completedOrders
      .filter((order) => {
        const orderDate = new Date(order.completedAt || order.updatedAt);
        return orderDate >= weekStart;
      })
      .reduce((sum, order) => sum + (order.quantity || 0), 0);
    weeklyGallonsEl.textContent = `${weeklyGallons} galon`;
  }

  const dailyGallonsEl = document.getElementById("couriers-daily-gallons");
  if (dailyGallonsEl) {
    const today = new Date().toDateString();
    const dailyGallons = completedOrders
      .filter((order) => {
        const orderDate = new Date(order.completedAt || order.updatedAt);
        return orderDate.toDateString() === today;
      })
      .reduce((sum, order) => sum + (order.quantity || 0), 0);
    dailyGallonsEl.textContent = `${dailyGallons} galon`;
  }
}
...
async function handleOrderSubmit(e) {
  ...
  try {
    ...
    const orderPrice = Number.parseInt(
      document.getElementById("order-price").value.replace(/[^\d]/g, "")
    );
    const orderQuantity = Number.parseInt(
...
function fillOrderEditForm(order) {
  ...
  document.getElementById("edit-returned-gallons").value =
    order.returnedGallons || order.quantity || 0;
...
function initializeCharts() {
  ...
  console.log("📊 Charts initialized and ready");
}
...
function updateCharts() {
  ...
  // Update orders chart
  ordersChart.data.labels = last7Days;
...
function updateAnalyticsData() {
  ...
  const avgOrder =
    completedOrders.length > 0
      ? completedOrders.reduce((sum, order) => sum + (order.price || 0), 0) /
...
function updateRecentDeliveries() {
  const recentDeliveriesContainer =
    document.getElementById("recent-deliveries");
  if (!recentDeliveriesContainer) return;
  ...
  if (recentOrders.length === 0) {
    recentDeliveriesContainer.innerHTML = `
      <div class="text-center py-8">
        <div class="text-gray-400 text-4xl mb-3">📦</div>
        <p class="text-gray-400">Belum ada pengantaran selesai</p>
      </div>
    `;
    return;
  }

  recentDeliveriesContainer.innerHTML = recentOrders
    .map(
      (order) => `
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-4 text-sm text-gray-400">
            <span>Kurir: ${order.assignedCourierName || "Unknown"}</span>
            <span>Jenis: ${order.waterType}</span>
            <span>Jumlah: ${order.quantity} galon</span>
          </div>
        </div>
      </div>
      <div class="text-right">
...
async function loadDeliveryHistory() {
  try {
    ...
    // Populate courier filter with assignedCourierName or courierName
    ...
    if (courierFilter) {
      ...
      const uniqueCouriers = [
        ...new Set(
          deliveryHistory
            .map((h) => h.assignedCourierName || h.courierName)
            .filter(Boolean)
        ),
      ];
    ...
    // Render all delivery history initially (without filter)
    renderDeliveryHistory(deliveryHistory);

    console.log(`✅ Loaded ${deliveryHistory.length} delivery history records`);
  } catch (error) {
...
window.filterDeliveryHistory = async () => {
  ...
  const courierEl = document.getElementById("history-courier");
  ...
  if (courierName) {
    filteredHistory = filteredHistory.filter(
      (h) => (h.assignedCourierName || h.courierName) === courierName
    );
  }
